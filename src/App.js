    ...
    const htmlEntries = entries.map((entry, i) => {
      return (
        <div className="entry" key={i}>
          <h3 className={`word${entry.error ? entry.error : ""}`}>
            {entry.word}
          </h3>
          <p
          ...
          <p className="wikilinks">
            <a
              href={`https://en.wikipedia.org/wiki/${
                entry.wikilink ? entry.wikilink : entry.word
              }`}
              target="_blank"
              rel="noreferrer noopener"
            >
              Visit Wikipedia - {entry.word}
            </a>
          </p>
        </div>
      );
    ...
    <div className={colorTheme}>
      ...
      <div className="header">
        <div className="container">
          ...
          <h5>
            Dictionary of words that are assumed knowledge in the defi community
            to help newbies.
          </h5>
        </div>
      </div>
      <div className="content">
        <div className="container">
          ...
          <input
            type="text"
            id="search"
            placeholder="Search in words and definitions here"
            value={searchTerm}
